...
#----------------------------------------------------------------------------
	 ...
	 */
	function isMovable() {
		return Namespace1::isMovable( $this->getNamespace() )
			&& $this->getInterwiki() == '';
	}
	 ...
	 */
	function isTalkPage() {
		return Namespace1::isTalk( $this->getNamespace() );
	}
	...
	/* private */ function secureAndSplit() {
		...
		do {
			if ( preg_match( "/^(.+?)_*:_*(.*)$/S", $t, $m ) ) {
				...
				if ( $ns = Namespace1::getCanonicalIndex( $lowerNs ) ) {
					# Canonical namespace
					$t = $m[2];
					$this->mNamespace = $ns;
	...
	function getTalkPage() {
		return Title::makeTitle( Namespace1::getTalk( $this->getNamespace() ), $this->getDBkey() );
	}
	...
	function getSubjectPage() {
		return Title::makeTitle( Namespace1::getSubject( $this->getNamespace() ), $this->getDBkey() );
	}
